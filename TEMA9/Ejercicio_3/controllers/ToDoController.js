...
router.route('/')
	...
	.post(function(req, res){
		...
		tarea.save(function(err, t){
			if(req.accepts('json')){
				...
				return res.status(201).json({message: "Salvado", _id: t._id, complete: JSON.stringify(t)});
			}
			else{
				res.send("No acepta JSON");
			}
	...
	.delete(function(req, res, next){
		...
		ToDo.findOneAndRemove({titulo: req.body.titulo}, null, function(err, doc, result){
			...
			return res.status(200).json({message: "Borrado", _id: doc});
		})
	});
...
router.put('/:id', function(req, res, next){
	console.log("body" , req.body)
	ToDo.findOneAndUpdate({_id: req.params.id}, {completa: req.body.completa}, null, function(err, doc, result){
		if(err){
			res.status(500).json({message: "Error editando la tarea", error: err});
		}
		return res.status(200).json({message: "Actualizada", _id: doc});
	})
});
...
router.get('/:id', function(req, res){
	...
	ToDo.findOne({_id:id}, function(err, tarea){
		if (req.accepts('json')){
			...
			if(!tarea){
				return res.status(500).json({message: "No se ha podido obtener la tarea"});
			}
			return res.status(200).json(tarea);
		}
		else{
			res.status(500).send("No acepta JSON");
		}
	});
});


router.get('/restart/all', function(req, res){
